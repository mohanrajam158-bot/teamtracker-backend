...
app.get("/tl-updates/unread-count", verifyToken, (req, res) => {
  ...
  db.query(sql, [req.user.team_id, req.user.id, req.user.id], (err, result) => {
    if (err) {
  ...
  if (!announcement_id) {
    return res.status(400).json({ error: "announcement_id required" });
  }

  db.query(
    `SELECT id FROM tl_announcements 
     WHERE id = ? AND team_id = ? LIMIT 1`,
    [announcement_id, req.user.team_id],
    (checkErr, rows) => {
      ...
      db.query(
        ...
        (err) => {
          ...
          res.json({ message: "One notification marked read ✅" });
        }
      );
    }
  );
